...
// import { Auth } from 'aws-amplify'; // Updated import to use Auth from 'aws-amplify'

// const fetchAuthSession = async () => {
//   return await Auth.currentSession(); // Fetching the current session using Auth
// };

// const signIn = async (credentials: { username: string; password: string }) => {
//   return await Auth.signIn(credentials.username, credentials.password); // Signing in using Auth
// };
...
//       console.log('User sub:', session.getAccessToken().decodePayload().sub);
      
//       Alert.alert('Auth Info', `Authenticated: ${session.isValid() ? 'Yes' : 'No'}\n\n${JSON.stringify(session, null, 2)}`);
//     } catch (error: any) {
